...
function affected(command, parsedArgs) {
    ...
    try {
        switch (command) {
            ...
            case 'print-affected':
                if (nxArgs.target) {
                    const projectWithTargetAndConfig = allProjectsWithTargetAndConfiguration(affectedProjects, nxArgs);
                    print_affected_1.printAffected(projectWithTargetAndConfig, affectedProjects, projectGraph, nxArgs, overrides);
                }
                else {
            ...
            case 'affected': {
                const projectWithTargetAndConfig = allProjectsWithTargetAndConfiguration(affectedProjects, nxArgs);
                shared_1.printArgsWarning(nxArgs);
                run_command_1.runCommand(projectWithTargetAndConfig, projectGraph, env, nxArgs, overrides, new default_reporter_1.DefaultReporter());
                break;
            }
        }
    }
...
function allProjectsWithTargetAndConfiguration(projects, nxArgs) {
    return projects.filter(p => project_has_target_and_configuration_1.projectHasTargetAndConfiguration(p, nxArgs.target, nxArgs.configuration));
}
