...
function updateProjectRootFiles(schema) {
    return (tree, _context) => {
        return rxjs_1.from(workspace_1.getWorkspace(tree)).pipe(operators_1.map(workspace => {
            ...
            const newRelativeRoot = path.relative(path.join(app_root_1.appRootPath, destination), app_root_1.appRootPath);
            const oldRelativeRoot = path.relative(path.join(app_root_1.appRootPath, project.root), app_root_1.appRootPath);
